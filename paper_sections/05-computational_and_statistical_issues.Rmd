...
For instance, the AIC for model 4 fit to this data is `r round(AIC(mod1))`, whereas it is `r round(AIC(mod2))` for model 5, implying a improvement in fit by allowing smoothness to vary.
...
Including a global smoother (models 2 and 3 compared to models 4 and 5) will not generally substantially affect the number of coefficients that need to be estimated (Table \ref{tab:comp_time_kable}).
