...
details). For models *GS* and *GI*, the global term is either entirely or almost entirely[^te_concurvity] concurved with the groupwise smoothers.
...
When fitting *GI* models (using `by=` terms for the groupwise-smoothers, or *GS* models using tensor products in **mgcv**, the global and group-level terms will not be entirely concurve because **mgcv** will automatically drop basis functions to ensure that these terms are not perfectly concurve, that is, so that no basis function in the global term could be formed from a linear combination of group-level basis functions (see `?mgcv::gam.side` for how terms to be dropped are selected).
Group-level terms fit `bs="fs"` smoothers will not have any basis functions dropped, as `mgcv` disables checking for side-constraints for these smoothers (since all basis functions are fully penalized for this type of smoother, in principle concurvity should not be an issue; see `?mgcv::smooth.construct.fs.smooth.spec` for details). 
...
To avoid this issue, we recommend both careful choice of basis and setting model degrees of freedom so that groupwise terms are either slightly less flexible than the global term or have a smaller null space. In the examples in section III, we used smoothers with an unpenalized null space for the global smoother and ones with no null space for the groupwise terms[^gsnull]. When using thin plate splines, this can be done with splines with a lower order of derivative penalized in the groupwise smoothers than the global smoothers, as lower-order "tp" splines have fewer basis functions in the null space. For example, we used `m=2` (penalizing squared second derivatives) for the global smoother, and `m=1` (penalizing squared first derivatives) for groupwise smoothers in models *GS* and *GI*. Another option is to use a lower number of basis functions (`k`) for groupwise relative to global terms.  This will reduce the maximum flexibility possible in the groupwise terms. We do caution that these are just rules of thumb. 

As noted above[^te_concurvity], interpreting the shape of global terms and group-wise deviations seperately for *GI* models and *GS* models fit using tensor-product group-level terms is complicated by the fact that **mgcv** will drop some basis-functions for the group-level term to prevent perfect concurvity. 
For tensor-product smoothers, `gam.side` will generally drop $\le k$ terms from the group-level smoother, where $k$ is the number of basis functions in the global smoother.
The total number of terms dropped will depend on the smoothers used for the global and groupwise terms, and will be dropped essentially at random from different grouping levels. 
This means that some groups will have a different range of potential deviations from the global smoother than others. 
For *GI* models fit with `by=` smoothers, `gam.side` will drop the same basis function for each group level, resulting in $g$ terms being dropped. 
This means that there will be some shapes of deviation that will not be possible to generate for these terms.
For both cases, this has the effect of also slightly altering the shape of the global smooth relative to what it would be based on model *G*; this will be a larger issue when the number of basis functions in the global smooth and the number of group levels are small. 

There is currently no way to disable dropping side constraints for these terms in **mgcv**. In cases where accurately estimating the global smoother or group-level deviations is essential, we recommend either fitting model *G*, *GS* using factor-smooth group-level terms (`bs="fs"`) or using specialized functional regression software such as the `fosr` function in the *refund* package [@scheipl_functional_2014], which does not impose these side constraints; instead the package uses a modified type of tensor-product to ensure that group-level terms sum to zero at each level of the predictor [@scheipl_functional_2014]. Also, see below for more information on functional regression.
