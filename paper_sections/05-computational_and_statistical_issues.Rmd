...
smoothers equal (models 2 & 4) will either lead to overly variable estimates for the
...
of among-group variability in smoothness when fitting model *GS* or 4 HGAMs. We
...
We then fit both model *S* (where all curves were assumed to be equally smooth) and model *M* (with varying smoothness) to the entire data set, using REML criteria to estimate penalties.
...
For instance, the AIC for model *S* fit to this data is `r round(AIC(mod1))`, whereas it is `r round(AIC(mod2))` for model *M*, implying a improvement in fit by allowing smoothness to vary.
...
It is possible to set up this type of varying penalty model in flexible Bayesian modelling software such as *Stan* (see below for a discussion of how to fit HGAMs using these tools), but how to fit this type of model has not been well studied. 
...
```{r single_smooth_bias_plot, echo=FALSE, fig.width=7, fig.height=6, message=FALSE, warning=FALSE, cache=TRUE, fig.cap="\\label{fig:var_pen} a) Illustration of bias that can arise from assuming equal smoothness for all group levels (model *S*, red line) versus allowing for intergroup variation in smoothness (model *M*, red line) when the true function (black line) shows substantial variation in smoothness between groups. b) Estimated wiggliness (as measured by the integral of the squared 2nd derivative) of the true function for each group level versus that for the functions estimated by model *S* (red) and model *M* (blue), indicating substantial undersmoothing for low-variability curves by model *S*.", dependson= -1}
#Plot the overfit graphs together.
cowplot::plot_grid(overfit_vis_plot, deriv_plot, ncol=1, labels="auto",
                   align="hv", axis="lrtb")
```

It may seem there is also a bias-variance trade-off between choosing to use a single global smoother (model *G*) or a global smoother plus group-level terms (models 2 and 3).
In model *G*, all the data is used to estimate a single smooth term, and thus should have lower variance than models 2 and 3, but higher bias for any given group in the presence of inter-group functional variability.
...
The choice between using model *G* versus models *GS* and *GM* should generally be driven by computational costs.
Model *G* is typically much faster to fit than models *GS* and *GM*, even in the absence of among-group differences.
If there is no need to estimate inter-group variability, model *G* will typically be
more efficient.

A similar issue exists when choosing between models *GS* and *GM* and models *S* and *M*.
If all group levels have very different functional shapes, the global term will get penalized toward zero in models *GS* and *GM*, so they will reduce to models *S* and *M*.
...
Including a global smoother (models *GS* and *GM* compared to models 4 and 5) will not generally substantially affect the number of coefficients that need to be estimated (Table \ref{tab:comp_time_kable}).
...
Adding additional smoothing parameters (moving from model *GS* to *GM*, or moving from model *S* to *M*) is more costly than increasing the number of coefficients to estimate, as estimating smoothing parameters is computationally intensive [@wood_fast_2011].
This means that models *GS* and *S* will generally be substantially faster than *GM* and *M* when the number of groups is large, as models *GM* and *M* fit a separate set of penalties for each group level. The effect of this is visible in comparing the time it takes to fit model *GS* to model *GM* (which has a smoother for each group) or models *S* and *M* for the example data (Table \ref{tab:comp_time_kable}). Note that this will not hold in all cases. For instance, model *M* takes less time to fit the bird movement data than model *S* does (Table \ref{tab:comp_time_kable}B).
...
First, there are a few limitations on how you are able to specify models 1-5 in both frameworks. Factor-smoother interaction (`bs="fs"`) basis setup works in both `gamm()` and `gamm4()`. However, as the **nlme** package does not support crossed random effects, it is not possible to have two factor-smoother interaction terms for the same grouping variable in `gamm()` models (e.g., `y~s(x1, grp, bs="fs")+s(x2, grp, bs="fs")`. These type of crossed random effects are allowed in **gamm4**. The use of `te()` terms are not possible in **gamm4**, due to
...
When fitting models with separate global and groupwise smoothers (models 2 and 3),
...
5 in Table \ref{tab:comp_time_kable}). These models can still be estimated
