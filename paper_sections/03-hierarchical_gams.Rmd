...
colinear with the null spaces of the groupwise smoothers (see section IV for
more discussion about the issue of colinearity and smoother selection).
...
Comparing models based on AIC is a more robust approach to comparing the different modelling approaches, as there is a well-developed theory of how to include smoothing effects when estimating the model complexity penalty for AIC [@wood_smoothing_2016]. We demonstrate this approach in Table \ref{tab:AIC_table}. Going by the AIC criteria, there is strong support for including among-group functional variability for both the CO2 dataset and the bird_move dataset (compare models 1 versus models 2-5). For the CO2 dataset (Table \ref{tab:AIC_table}A), there is relatively strong evidence that there is more intergroup variability in smoothness than model 2 allows, and weaker evidence that model 4 or 5 (separate smooths for all plants) show the best fit. For the bird_move dataset (Table \ref{tab:AIC_table}B), model 2 (global smooth plus group-level smooths with a shared penalty) fits the data best, which is good as that is how we simulated the data! We have also included the adjusted $R^2$ for all models in Table \ref{tab:AIC_table}, but we do not recommend selecting a model based on this, for the same reasons that we do not recommend parametric p-values. 


```{r AIC_table, echo=F,  fig.width=4, fig.height=6,message=F, warning=F, cache=T}
get_rsqr = function(model_list){
  rsqr = rep(0, times= length(model_list))
  for(i in 1:length(model_list)){
    rsqr[i] = summary(model_list[[i]])$r.sq
  }
  rsqr
}
...
AIC_table = AIC(CO2_mod1,CO2_mod2, CO2_mod3, CO2_mod4, CO2_mod5,
  ...
  mutate(`Adjusted R squared` = get_rsqr(list(CO2_mod1,CO2_mod2, CO2_mod3, CO2_mod4,
                                     CO2_mod5,bird_mod1, bird_mod2, bird_mod3,
                                     bird_mod4, bird_mod5)),
         `Adjusted R squared` = round(`Adjusted R squared`, digits = 2))%>%
  mutate_at(.vars = vars(df,AIC), .funs = funs(round,.args = list(digits=0)))


kable(AIC_table,format ="latex", caption="AIC table comparing CO2 and bird_move model fits.")%>% #NOTE: change format to "latex" when compiling to pdf, "html" when compiling html
