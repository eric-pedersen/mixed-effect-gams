...
```{r Fig6, echo=FALSE, fig.width=6, fig.height=3, dev.args=list(pointsize=10), fig.cap="\\label{fig:co2_modG}**gratia** plotting output for model *G* applied to the CO2 dataset. `s(log(conc)): the smoother of $\\log_e$ concentration. `Plant_uo`: a quantile-quantile plot of the random effects against Gaussian quantiles, used to check the appropriateness of the normal random effect assumption."}
<<co2_modG_unrun>>
...
```{r Fig8, echo=FALSE, fig.width = 5, fig.height = 4, outwidth="\\linewidth", dev.args = list(pointsize=10), fig.cap="\\label{fig:bird_modG} Plot illustrating the average log-abundance of all bird species at each latitude for each week, with red colours indicating more individuals and blue colours fewer."}
...
```{r Fig10, fig.width=6, fig.height=3, dev.args=list(pointsize=10), out.width="\\linewidth", echo=FALSE, fig.cap="\\label{fig:co2_modGS}Global function (`s(log(conc))`) and group-specific deviations from the global function (`s(log(conc),Plant_uo)`) for \\texttt{CO2\\_modGS}"}
...
```{r Fig11,echo=FALSE, fig.width=6, fig.height=4, out.width="\\linewidth", fig.cap="\\label{fig:co2_modGS_pred}Predicted uptake values (lines) versus observed uptake for each plant, based on model \\emph{GS}."}
...
```{r Fig12, fig.width=8, fig.height=4, echo=FALSE, out.width="\\linewidth", fig.cap="\\label{fig:bird_modGS}a) Predicted migration paths for each species based on \\texttt{bird\\_modGS}, with lighter colors corresponding to higher predicted counts. b) Observed counts versus predictions from \\texttt{bird\\_modGS}."}
...
```{r Fig13, fig.width=9, fig.height=5, echo=FALSE, fig.cap="\\label{fig:co2_modGI}Functional relationships for the CO2 data estimated for model \\emph{GI}. `s(log(conc))`: the global smoother; `Plant_uo`: species-specific random effect intercepts. The remaining plots are a selected subset of the plant-specific smoothers, indicating how the functional response of that plant differs from the global smoother."}
