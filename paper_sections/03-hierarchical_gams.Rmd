...
```{r bird_mod1, echo=TRUE,  fig.width=6, fig.height=6, message=F, warning=F, cache=TRUE}
library(mgcv)
library(ggplot2)
library(tidyr)
...
bird_mod1 = gam(count ~ te(latitude, week, bs= c("tp","cc"), k=c(9,9)), data= bird_move, method="REML", family= poisson)

plot(bird_mod1, page=1, scheme=2)

```

```{r bird_mod1_ggplot, echo=TRUE,  fig.width=8, fig.height=3, message=F, warning=F, cache=TRUE}
...
ggplot(data=bird_move_plot, aes(x=week, y=latitude, fill = value,color=value))+
  ...
  scale_x_continuous(expand=c(0,0),breaks = c(12,24,36,48))+
