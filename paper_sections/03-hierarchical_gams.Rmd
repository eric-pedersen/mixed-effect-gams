...
```{r modGI_CO2, fig.width=5, fig.height=3, echo=FALSE, fig.cap="\\label{fig:co2_modGI}Functional relationships for the CO2 data estimated for model *GI*. Top left: the global smoother; Top middle: species-specific random effect intercepts. The remaining plots are a selected subset of the plant-specific smoothers, indicating how the functional response of that plant differs from the global smoother. "}
CO2_modGI <- gam(log(uptake) ~ s(log(conc), k=5, m=2, bs="tp") +
                  s(log(conc), by= Plant_uo, k=5, m=1, bs="tp") +
                  s(Plant_uo, bs="re", k=12),
                data=CO2, method="REML")
...
The fitted model for `bird_modGI` is visually indistinguishable from `bird_modGS` (figure \ref{fig:bird_modGS}) so we do not illustrate it here. 
