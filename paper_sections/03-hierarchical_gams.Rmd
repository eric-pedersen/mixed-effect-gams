...
```{r co2_mod1, echo=TRUE,  fig.width=6, fig.height=3,message=F, warning=F, cache=T}
library(mgcv)
library(ggplot2)
CO2$Plant = factor(CO2$Plant, levels = levels(CO2$Plant), ordered = F)

CO2_mod1 = gam(log(uptake) ~ s(log(conc),k=7,m=2, bs="tp")+s(Plant, k =12,  bs="re"), 
...
CO2_penmat = create_connected_pmat(CO2$Plant)

CO2_mod2a = gam(log(uptake) ~ s(log(conc),k=6,m=2, bs="tp")+
                  s(log(conc), Plant, k =6,  bs="fs",m=1), 
                data= CO2,method="REML")

CO2_mod2b = gam(log(uptake) ~ te(log(conc),Plant, k=c(6,12),m=2, 
                                 bs=c("tp","mrf"), xt= list(penalty = CO2_penmat)), 
                data= CO2,method="REML")

plot(CO2_mod2a,page=1)
```



The above plot shows the global function (left) and group-specific deviations from the global function (right) for `CO2_mod2a`. We did not include a call to the plot function for `CO2_mod2b` as there is no default plotting method for a tensor product of a mrf smooth and a continous smoother. However, as is visible below, both smoothers have very similar behaviour when predicting group-specific variation: 
...
The model 2a fit is shown in black, and the model 2b fit is shown in red. As mentioned above, both models show similar fit to the data.
...
plot(bird_mod2a, page=1, scheme=2,rug = F)
...
```{r mod3, echo=TRUE,  fig.width=6, fig.height=3,message=F, warning=F, cache=T}


CO2_mod3 = gam(log(uptake) ~ s(log(conc),k=6,m=2, bs="tp")+
                  s(log(conc),by= Plant, k =6,  bs="tp",m=2), 
                data= CO2,method="REML")
...
CO2_mod4 = gam(log(uptake) ~ s(log(conc), Plant, k =6,  bs="fs",m=2), 
                data= CO2,method="REML")
...
CO2_mod5 = gam(log(uptake) ~ s(log(conc),by= Plant, k =6,  bs="tp",m=2)+s(Plant,bs="re",k=12), 
                data= CO2,method="REML")
