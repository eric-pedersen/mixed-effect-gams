...
So far we have focused on uisng HGAMs in a frequentist framework, where the goal
is to estimate single curves that best approximate some underlying true curve.
However, both HGLMs and GAMs, and consquently HGAMs, fit comfortably into a
...
take a specied model formula and automatically convert it into code for the JAGS
(or BUGS) Bayesian statistical packages, which can be adapted by the user to
their own needs. Similiarly, the `brms` package [@burkner_brms:_2017], which can
