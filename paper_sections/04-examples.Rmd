...
useful, so we could could borrow information from the more common species). As the data are seasonal, we use cyclic smoothers as the basis for seasonal dynamics.  Therefore we need to specific start and end points for our cycles using the `knots` argument to `gam`, as well as specify that this is smoother type to the factor-smooth using the `xt` argument:
...
out_format <- ifelse(knitr::opts_knit$get("rmarkdown.pandoc.to") == "pdf",
                    "latex",
                     ifelse(knitr::opts_knit$get("rmarkdown.pandoc.to") == "html",
                            "html",
                            NA_character_)
                      )

kable(zoo_test_summary, format = out_format, caption="Out-of-sample predictive ability for model 4 and 5 applied to the zooplankton community dataset. MSE values represent the average squared difference between model predictions and observations for test data.", booktabs = T)%>%
...
kable(daph_test_summary,format =out_format, caption="Out-of-sample predictive ability for model 1-3 applied to the *D. mendotae* dataset. MSE values represent the average squared difference between model predictions and observations for held-out data (zero predictive ability would correspond to a MSE of one).", booktabs = T)%>%
  kable_styling(full_width = F)
```
