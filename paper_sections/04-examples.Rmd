...
```{r zoo_comm_modM, echo=TRUE, message=FALSE,  cache=TRUE, fig.width=8, fig.height=5}
# Note that  s(taxon, bs="re") has to be explicitly included here, as the 
# day  by taxon smoother does not include an intercept
zoo_comm_modM <- gam(density_adj ~ s(day, by=taxon,
                                     k=10, bs="cc") + 
                                   s(taxon, bs="re") +
                                   s(taxon, year_f, bs="re"),
                     data=zoo_train,
...
round(k.check(zoo_comm_modM),2)
```


```{r zoo_comm_check_k_kable, echo=FALSE, message=FALSE,  cache=TRUE,purl=FALSE}

kable(round(k.check(zoo_comm_modM),2), 
      format = table_out_format, 
      caption="Results from running `k.check` on zoo_comm_modM.", 
      booktabs = TRUE,
      escape = FALSE)%>%
...
```{r zoo_comm_diag, echo=FALSE, message=FALSE, warning=TRUE, cache=TRUE,results="markup", fig.width=6, fig.height=3, fig.cap = "\\label{fig:zoo_comm_diag_plot} Diagnostic plots for model *M* fitted to zooplankton community data in Lake Mendota. Top row: QQ plots plus the 1-1 line. Bottom row: deviance residuals versus fitted values (on the link scale)."}
#individual components of gam.check: residual plots
par(mfrow= c(1,2))
qq.gam(zoo_comm_modS)
plot(log(fitted(zoo_comm_modM)), 
     residuals.gam(zoo_comm_modM,type = "deviance"), 
     xlab = "linear predictor",
     ylab = "residuals")
```


Differences between models 4 (shared smoothness between taxa) and 5 (different smoothness for each taxa) seem to be driven by the low seasonality of *L. siciloides* relative to the other species, and how this is captured by the more flexible model *M* (Figure \ref{fig:zoo_comp}). Still, both models show very similar fits to the training data. Model *S* is slightly better at predicting out of sample fits for all taxa except *M. edax* (Table \ref{tab:zoo_comm_outofsample_kable}). Even though *L. siciloides* showed weak seasonal dynamics, Model *S* and *I* still did a better job of predicting the density of this species out of sample compared to a simple model with only a species-specific intercept (Table \ref{tab:zoo_comm_outofsample_kable}).

```{r zoo_comm_plot, echo=FALSE, message=FALSE, warning=TRUE, cache=TRUE,results="markup", fig.width=6, fig.height=6, fig.cap = "\\label{fig:zoo_comp}Species-specific seasonal dynamics for the eight zooplankon species tracked in Lake Mendota. Black points indicate individual plankton observations. Lines indicate predicted average values for model *S* (black) and model *M* (red). Ribbons indicate $\\pm$ 2 standard errors around the mean."}
...
zoo_modM_fit <- predict(zoo_comm_modM, 
                        zoo_plot_data, 
                        se.fit = TRUE, 
                        exclude = "s(taxon,year_f)")

zoo_plot_data$modS_fit <- as.numeric(zoo_modS_fit$fit)
zoo_plot_data$modM_fit <- as.numeric(zoo_modM_fit$fit)

zoo_plot_data <- gather(zoo_plot_data, model, fit, modS_fit, modM_fit)
zoo_plot_data <- mutate(zoo_plot_data, se= c(as.numeric(zoo_modS_fit$se.fit),
                                             as.numeric(zoo_modM_fit$se.fit)),
                        upper = exp(fit + (2 * se)),
                        lower = exp(fit - (2 * se)),
                        fit   = exp(fit))
...
zoo_test_summary = zoo_test %>%
  mutate(
    ...
    modM = predict(zoo_comm_modM, ., type="response"))%>%
  group_by(taxon)%>%
  summarise(
    ...
    `Model S` = format(get_RMSE(modS, density_adj), 
                       scientific = FALSE, 
                       digits=3),
    `Model M` = format(get_RMSE(modM, density_adj), 
                       scientific = FALSE, 
                       digits=3))%>%
...
Here, we will compare models *G*, *GS*, and *GM* to determine if a single global function is appropriate for all four lakes, or if we can more effectively model variation between lakes with a shared smoother and lake-specific smoothers.
...
```{r zoo_daph_modGM, echo=TRUE, message=FALSE,  cache=TRUE}
zoo_daph_modGM <- gam(density_adj~s(day, bs="cc", k=10) +
...
The AIC values indicate that both model *GS* (`r round(AIC(zoo_daph_modGS), 2)`) and 3
(`r round(AIC(zoo_daph_modGM), 2)`) are better fits than model *G* (`r round(AIC(zoo_daph_modG), 2)`),
but models 3 fits somewhat better than model *GS* [^AIC_note].  There does not seem to be a large amount of inter-lake variability (the effective degrees of freedom per lake are low in models 2 & 3).  Plots for all three models
...
```{r zoo_daph_plot, echo=FALSE, message=FALSE, warning=TRUE, cache=TRUE, fig.width=6, fig.height=4, fig.cap="\\label{fig:daph_smooth}Raw data (points) and fitted models (lines) for \\textit{D. mendota} data. Green: model *G* (no inter-lake variation in dynamics); orange: model *GS* (interlake variation with similar smoothness); purple: model *GM* (varying smoothness among lakes). Shaded bands are drawn at $\\pm$ 2 standard errors around each model."}
#Create synthetic data to use to compare predictions
...
daph_modGS_fit <- predict(zoo_daph_modGS, 
                         ...
                         se.fit = TRUE, 
                         exclude = "s(lake,year_f)")
daph_modGM_fit <- predict(zoo_daph_modGM, 
                         newdata = daph_plot_data, 
                         se.fit = TRUE, 
                         exclude = "s(lake,year_f)")
...
daph_plot_data$modGM_fit <- as.numeric(daph_modGM_fit$fit)
...
daph_plot_data <- mutate(daph_plot_data, 
                         se = c(as.numeric(daph_modG_fit$se.fit),
                                ...
                                as.numeric(daph_modGM_fit$se.fit)),
                         upper = exp(fit + (2 * se)),
                         lower = exp(fit - (2 * se)),
                         fit   = exp(fit))
...
Model *GM* is able to predict as well as model *G* or *GS* (Table \ref{tab:zoo_daph_outofsample_kable}), indicating that allowing for inter-lake variation in seasonal dynamics improved model prediction. None of the models did well in terms of predicting Lake Kegonsa or Lake Waubesa dynamics out of sample compared to a simple model with only a lake-specific intercept and no intra-annual variability (Table \ref{tab:zoo_daph_outofsample_kable}). 
...
daph_test_summary <- daphnia_test %>%
  mutate(#get out-of-sample predicted fits
    ...
    modGM = as.numeric(predict(zoo_daph_modGM,.,type="response")))%>%
  group_by(lake)%>%
  summarise(`Intercept only` = format(get_RMSE(mod0, density_adj), 
            ...
            `Model GS` = format(get_RMSE(modGS, density_adj), 
                               scientific = FALSE, digits=2),
            `Model GM` = format(get_RMSE(modGM, density_adj), 
                               scientific = FALSE, digits=2))%>%
  rename(Lake = lake)
```

```{r zoo_daph_outofsample_kable, echo=FALSE, message=FALSE,  cache=TRUE, purl = FALSE}
kable(daph_test_summary,format = table_out_format, caption="Out-of-sample predictive ability for model *G*,*GS*, and *GM* applied to the \\textit{D. mendotae} dataset. RMSE values represent the average squared difference between model predictions and observations for held-out data.", booktabs = TRUE)%>%
