...
the group-level signals (cosine waves) constant. The true function for
each group level (black lines) shows substantial variation in smoothness
between groups. b) Wiggliness (as measured by the integral of the
squared 2nd derivative) of the estimated function for each replicate for
each group level for model \emph{S} (red) and model \emph{I} (blue),
versus the true wiggliness of the function for that grouping level, with
the black line indicating the one-to-one line. Points below (above) the
black line indicate that a given model estimated the curve as less
(more) wiggly than the true curve used to generate the data.}
\end{figure}
...
\hspace{1em}GI & 3 & 65 & 3\\
\hspace{1em}S & 12 & 61 & 13\\
\hspace{1em}I & 2 & 61 & 3\\
\addlinespace[0.3em]
\multicolumn{4}{l}{\textbf{B. bird movement data}}\\
\hspace{1em}G & 1 & 90 & 2\\
\hspace{1em}GS & 490 & 624 & 14\\
\hspace{1em}GI & 290 & 540 & 5\\
\hspace{1em}S & 150 & 535 & 12\\
\hspace{1em}I & 160 & 541 & 3\\
\bottomrule
\end{tabular}
\end{table}
