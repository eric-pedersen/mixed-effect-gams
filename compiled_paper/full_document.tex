...
deviations from that totally smooth function (more on this in section
II).
...
\textbf{mgcv}. We have also included all the code needed to make the
figures for this document in supplemental code (online), and on the
...
is then added to the model likelihood, controlling the trade-off via a
\emph{smoothing parameter} (\(\lambda\)). The penalized likelihood used
to fit the model is thus: \[
...
choices of the smoothing parameter (\(\\lambda\)) effect the shape of
the resulting smoother. Data (points) were generated from the blue
function and noise added to them. In the left plot \(\\lambda\) was
estimated using Restricted Maximum Likelihood (REML) to give a good fit
to the data, in the middle plot \(\\lambda\) was set to zero, so the
penalty has no effect and the function interpolates the data, the right
plot shows when \(\\lambda\) is set to a very large value, so the
penalty removes all terms that have any wiggliness, giving a straight
line.
...
\begin{figure}
\includegraphics[width=\linewidth]{full_document_files/figure-latex/lambda-1} \caption{\label{fig:smoothing_effect}Effect of different choices of smoothing parameter ($\lambda$) on the shape of the resulting smoother. Left:  $\lambda$ estimated using REML; middle: $\lambda$ set to zero (no smoothing); Right: $\lambda$ is set to a very large value.}\label{fig:lambda}
\end{figure}
...
\begin{figure}
\includegraphics[width=\linewidth]{full_document_files/figure-latex/basis_function_examples-1} \caption{\label{fig:basis_example}a) Examples of the basis functions associated with a six basis function thin plate spline (m=2), calculated for data, $x$, spread evenly between $x=0$ and $x=1$. Each line represents a single basis function. b) The smoothing  penalty matrix for the thin plate smoother. Red entries indicate positive values and blue indicate negative values.  For example, functions F3 and F4 would have the greatest proportionate effect on the total penalty (as they have the largest values on the diagonal), whereas function F5 and F6 would not contribute to the wiggliness penalty at all (all the values in the 5th and 6th row and column of the penalty matrix are zero). This means these functions are in the null space of this basis, and are treated as completely smooth. c) An example of how the basis functions add up to create a single smooth function. Thin coloured lines represent each basis function multiplied by a coefficient, and the solid black line is the sum of those basis functions.}\label{fig:basis_function_examples}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{full_document_files/figure-latex/co2_mod1-1.pdf}
...
\begin{figure}
\centering
\includegraphics{full_document_files/figure-latex/bird_mod1_plot-1.pdf}
...
\begin{figure}
\includegraphics[width=\linewidth]{full_document_files/figure-latex/co2_mod2_plot-1} \caption{\label{fig:co2_mod2}Global function (left) and group-specific deviations from the global function (right) for \texttt{CO2\_mod2}}\label{fig:co2_mod2_plot}
\end{figure}
...
\begin{figure}
\includegraphics[width=\linewidth]{full_document_files/figure-latex/co2_mod2_ggplot-1} \caption{\label{fig:co2_mod2_pred}Predicted uptake values (lines) versus observed uptake for each plant, based on model 2.}\label{fig:co2_mod2_ggplot}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{full_document_files/figure-latex/mod3_CO2-1.pdf}
...
\caption{\label{tab:AIC_table}AIC table comparing model fits for example datasets}
\centering
...
\hspace{1em}bird\_mod1 & 47 & 3433 & 1899\\
\hspace{1em}bird\_mod2 & 140 & 1534 & 0\\
\hspace{1em}bird\_mod3 & 248 & 1681 & 147\\
\hspace{1em}bird\_mod4 & 143 & 1543 & 9\\
\hspace{1em}bird\_mod5 & 201 & 1602 & 68\\
\bottomrule
\end{tabular}
\end{table}
...
theoretical quantiles (Figure \ref{fig:zoo_comm_diag_plot} top). There
also does not seem to be a pattern in the residual versus fitted values
(Figure \ref{fig:zoo_comm_diag_plot} bottom), except for a line of
...
## s(day):taxonC. sphaericus        8  4.78    0.89    0.48
## s(day):taxonCalanoid copepods    8  6.66    0.89    0.42
## s(day):taxonCyclopoid copepods   8  5.31    0.89    0.44
## s(day):taxonD. mendotae          8  6.95    0.89    0.48
## s(day):taxonD. thomasi           8  6.57    0.89    0.45
## s(day):taxonK. cochlearis        8  5.92    0.89    0.45
## s(day):taxonL. siciloides        8  0.52    0.89    0.45
...
\begin{figure}
\centering
\includegraphics{full_document_files/figure-latex/zoo_comm_diag-1.pdf}
...
\begin{figure}
\centering
\includegraphics{full_document_files/figure-latex/zoo_comm_plot-1.pdf}
...
\caption{\label{tab:zoo_comm_outofsample}Out-of-sample predictive ability for model 4 and 5 applied to the zooplankton community dataset. RMSE values represent the square root of the average squared difference between model predictions and observations for test data.  Intercept only results are for a null model with only year and year-by taxon random effect intercepts included.}
\centering
...
\begin{figure}
\centering
\includegraphics{full_document_files/figure-latex/zoo_daph_plot-1.pdf}
...
\caption{\label{tab:zoo_daph_outofsample}Out-of-sample predictive ability for model 1-3 applied to the \textit{D. mendotae} dataset. RMSE values represent the average squared difference between model predictions and observations for held-out data (zero predictive ability would correspond to a RMSE of one).}
\centering
...
to fix this type of model has not been well studied.

\begin{figure}
\centering
\includegraphics{full_document_files/figure-latex/single_smooth_bias_plot-1.pdf}
...
\hspace{1em}1 & 1 & 17 & 2\\
\hspace{1em}2 & 7 & 65 & 3\\
\hspace{1em}3 & 14 & 65 & 14\\
\hspace{1em}4 & 4 & 61 & 3\\
\hspace{1em}5 & 14 & 61 & 13\\
\addlinespace[0.3em]
\multicolumn{4}{l}{\textbf{B. bird movement data}}\\
\hspace{1em}1 & 1 & 90 & 2\\
\hspace{1em}2 & 120 & 540 & 5\\
\hspace{1em}3 & 140 & 624 & 14\\
\hspace{1em}4 & 110 & 541 & 3\\
\hspace{1em}5 & 67 & 535 & 12\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}
\centering
\includegraphics{full_document_files/figure-latex/alt_model_timing_plot-1.pdf}
...
work started from. The research was supported by the National Science
and Engineering Research Council of Canada (NSERC), Fisheries and Oceans
Canada and \textbf{add other support here}. DLM was partly funded by
OPNAV N45 and the SURTASS LFA Settlement Agreement, managed by the U.S.
