...
overfit_predict_data = crossing(x = seq(0,2*pi,length=500), freq = freq_vals)%>%
  mutate(grp = factor(freq, levels = freq_vals),
         y = sin(freq*x))%>%
...
overfit_predict_data_long = overfit_predict_data %>%
  ...
  mutate(model = recode(model, y = "true value",fit1 = "independent smooths, shared penalty\n(model 4)",
                        fit2 = "independent smooths and penalties\n(model 5)"),
         model = factor(model, levels=  c("true value","independent smooths, shared penalty\n(model 4)", 
                                          "independent smooths and penalties\n(model 5)")))
...
deriv_est_data = overfit_predict_data%>%
  ...
  mutate(model = factor(ifelse(model=="fit1_int", "independent smooths, shared penalty\n(model 4)",
                               "independent smooths and penalties\n(model 5)"),
                        levels = c("independent smooths, shared penalty\n(model 4)", 
                                   "independent smooths and penalties\n(model 5)")))


deriv_plot =  ggplot(data=deriv_est_data, aes(x=sqr_2nd_deriv, y= obs_sqr_deriv,color= model))+
  geom_point()+
  scale_y_log10("integral of squared second derivative\nfor fitted curves")+
  scale_x_log10("integral of squared second derivative\nfor true curve")+
