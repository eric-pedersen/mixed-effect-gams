...
comp_resources[1,"time"] = system.time(CO2_mod1 <- gam(log(uptake) ~ s(log(conc),k=7,m=2, bs="tp")+s(Plant_uo, k =12,  bs="re"), 
...
comp_resources[3,"time"] = system.time(CO2_mod2a <- gam(log(uptake) ~ s(log(conc),k=6,m=2, bs="tp")+
