...
main_func = generate_smooth_func(x,1,main_func_scale,main_func_amp)
indiv_func = generate_smooth_func(x,n_groups,indiv_func_scale,
                                  indiv_func_amp)
indiv_perturbs = exp(rnorm(n_groups,0,indiv_mult))
...
for(i in 1:n_groups){
  full_func[,i] = indiv_perturbs[i]*(full_func[,i]+main_func[,1])
}


colnames(full_func) = paste("G",1:n_groups,sep="")
matplot(x, full_func,type='l')
points(x, main_func[,1],type="l",lwd= 3)
...
print(plot_grid(random_fit_plot, block_fit_plot,ncol= 2))
